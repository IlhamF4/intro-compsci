...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """
    uniq = list(freq_dict1.keys()) + [i for i in freq_dict2 if i not in freq_dict1.keys()]
    num = 0
    denom = 0
    for i in uniq:
    	num += abs(freq_dict1.get(i, 0) - freq_dict2.get(i,0))
    	denom += freq_dict1.get(i,0) + freq_dict2.get(i,0)
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    """
    #create a union of frec_dict1 and freq_dict2
    union = freq_dict1
    for i in freq_dict2.keys():
    	if i not in union:
    		union += i
    	elif i in union:
    		union[i] = union[i] + freq_dict2[i]
    max = 0
    word = []
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    word_dict = get_frequencies(wordlist)
    
    #create dictionary for tf value
    tf_dict = {}
    for k,v in word_dict.items():
    	tf_dict[k] = v/len(wordlist)
    return tf_dict
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    """
    wordlist = [text_to_list(load_file(i)) for i in file_paths]
    ...
    for k in keylist:
    	...
    	for i in wordlist:
    		if k in i:
    			count +=1
    	idf_dict[k] = math.log10(len(wordlist)/count)
    return idf_dict
