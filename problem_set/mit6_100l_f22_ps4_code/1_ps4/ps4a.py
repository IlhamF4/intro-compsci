...
def find_tree_height(tree):
    ...
    if tree is None:
    	return 0
    elif tree.get_left_child() is None and tree.get_right_child() is None:
    	return 0
    else:
...
def is_heap(tree, compare_func):
    ...
    if tree is None:
    	...
    	left = tree.get_left_child() is None or compare_func(tree.get_left_child().get_value(),tree.get_value())
    	right = tree.get_right_child() is None or compare_func(tree.get_right_child().get_value(),tree.get_value())
    	return left and right and is_heap(tree.get_left_child(),compare_func) and is_heap(tree.get_right_child(),compare_func)
